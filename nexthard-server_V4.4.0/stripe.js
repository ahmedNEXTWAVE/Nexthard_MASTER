const stripe = require("stripe")("sk_test_51PjrLWFdLMYmYbzpLW8dJNJ8TlHeU8OR9Hms98TjqSNl7VEr7p5q90FH0OlcxyxaUr1FfpI3fPkoAsFAX3qAtSNE00DACVySpl");
const { publish_response }  = require('./mqttClient.js');
  ...
  module.exports.processPayment = async function(readerId,amount) {
    try {
      ...
      const confirmedPaymentIntent = await confirmPaymentIntent(paymentIntent.id, readerId);
...
if (retries<1)
{
  ...
  const paymentIntent2 = await stripe.paymentIntents.retrieve(paymentIntent.id);
